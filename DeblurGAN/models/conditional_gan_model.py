...
class ConditionalGAN(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.netG = networks.define_G(
            ...
            not opt.no_dropout, self.gpu_ids, use_parallel, opt.learn_residual, opt
        )
        if self.isTrain:
            use_sigmoid = opt.gan_type == 'gan'
            self.netD = networks.define_D(
                opt.output_nc, opt.ndf, opt.which_model_netD,
                opt.n_layers_D, opt.norm, use_sigmoid, self.gpu_ids, use_parallel, opt
            )
    ...
    def get_current_visuals(self):
        ...
        return OrderedDict([('input_label', real_A), ('synthesized_image', fake_B), ('gt', real_B)])
